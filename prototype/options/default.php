<html>
	<head>
		<link rel="stylesheet" type="text/css" href="/3/css/global.css">
		<link rel="stylesheet" type="text/css" href="/3/css/options.css">
		<?

	$root = "../../";
	require_once $root."server_references.php";

	$user = GetAuthorizedUser(true);

	require_once $root."references.php"; 
	

	?>		<title>Меню пользователя</title>
		<script language="javascript" src="/3/js1/wysiwyg/tiny_mce.js"></script>
	</head>

	<body onload="OnLoad()">
		<div id="AlertContainer">
			<table border="0" cellpadding="0" cellspacing="0" width="100%" height="100%">
				<tr><td align="center" valign="middle">
					<div id="AlertBlock">
					</div>
				</td></tr></table></div>

		<div id="OptionsContainer">
			<div id="OptionsContent">
		<? 
			if ($user->IsEmpty()) {
		?>
				<div class="Error">Пользователь не авторизован!</div>
		<?
			} else {
				echo "<h1>Меню пользователя ".$user->User->Login."</h1>";
			}
		?>
			</div>
	   	</div>
		<script src="/3/js1/options.js"></script>
	</body>
</html>