...
JsQuote($this->Status->Color)."\",".self::IsIgnoredDefault.",".self::IgnoresYouDefault.")";
  ...
  function ToJSON($status) {
    ...
    return array(
      ...
      $this->User->Login,
      $this->User->RoomId,
      Boolean($this->User->RoomIsPermitted),
      $status->Rights > $AdminRights ? $this->User->SessionAddress : "",
      $this->User->IsAway() ? " ".$this->User->AwayMessage : "",
      $this->User->BanReason,
      $this->User->BannedBy,
      $this->Nickname->Title,
      ...
      $this->Status->Title,
      $this->Status->Color,
      Boolean(self::IsIgnoredDefault),
      Boolean(self::IgnoresYouDefault)
     );
  }
