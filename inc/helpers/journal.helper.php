<?
    ...
    function TagCutLink($caption) {
      global $cutCount;
    	...
    	return "<span class='cutlink'>(<a href='##LINK###cut".$cutCount++."'>".($caption ? $caption : "œÓ‰Ó·ÌÂÂ")."</a>)</span>";
    }
	...
	function InjectionProtection($text) {
        $text = preg_replace("/( |:)on([a-z ]+=)/i", "\\1Ón\\2", $text);
		...
		$text = preg_replace("/\<script/i","\<scr˚pt", $text );
		$text = preg_replace("/iframe/i","˚frame", $text );
		$text = preg_replace("/<frame/i","<fr‡me", $text );
		$text = preg_replace("/display( *)[:=]/i","displ‡y:", $text );
		$text = preg_replace("/absolute/i","absÓlute", $text );
		$text = preg_replace("/scroll( *)=( *)no/i","scroll=Û„Û", $text );
	  ...
	  global $usedTags;
		...
		if ($message->IsCommentable) {
			$comments = JournalComment::MakeLink(
				...
				$userUrlName,
				0, 
				Countable("ÍÓÏÏÂÌÚ‡ËÈ", $commentsCount, "ÌÂÚ"));
		}
		$result = str_replace(
			"##COMMENTS##", 
			$comments,
		...
		if (strpos($result, "##TAGS##") !== false) {
			...
			$result = str_replace("##TAGS##", $tags ? "“Â„Ë: ".$tags : "", $result);
		}
