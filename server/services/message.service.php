		...
		if ($recepients) {
					...
					case "kick":
						if ($user->IsAdmin() || $user->Status->Rights == $KeeperRights) {
							...
							if ($user->Status->Rights >= $targetStatus->Rights) {
								...
								$msg = new SystemMessage($user->DisplayedName()." выгон€ет из чата ".Clickable($n).($message ? " &laquo;".$message."&raquo;" : "."), $user->User->RoomId);
					...
					case "ban":
						if ($user->IsAdmin()) {
							...
							if ($user->Status->Rights > $targetStatus->Rights && $targetStatus->Rights != $KeeperRights) {
								...
								$targetUser->Save();
								$msg = new SystemMessage($user->DisplayedName()." банит пользовател€ ".Clickable($n).($message ? " &laquo;".$message."&raquo;" : "."), $user->User->RoomId);
			...
			switch ($type) {
					...
					if ($roomExists) {
						if ($user->Status->Rights >= $TopicRights && (!$room->TopicLock || $user->IsAdmin())) {
							if ($message || $type == "topic") {
								...
								$msg = new SystemMessage(Clickable($user->DisplayedName())." мен€ет тему на &laquo;".$message."&raquo;.", $user->User->RoomId);
							}
							$room->Save();
						} else {
							$msg = new PrivateSystemMessage("»зменение темы невозможно!", $user->User->RoomId, $user->User->Id);
				...
				case "away":
					...
					$user->User->Save();
					$msg = new SystemMessage(Clickable($user->DisplayedName())." отлучаетс€ из чата. ".($message ? " &laquo;".$message."&raquo;" : ""), $user->User->RoomId);
					break;
				case "quit":
					$text = "";
					if (!$message) {
			...
			if ($msg) {
				if ($type != "away" && $user->User->AwayTime) {
					$backMsg = new SystemMessage(Clickable($user->DisplayedName())." возвращаетс€".($user->User->AwayMessage ? ", закончив &laquo;".$user->User->AwayMessage."&raquo;" : " в чат").", отсутствовав с ".PrintableTime($user->User->AwayTime), $user->User->RoomId);
					$backMsg->Save();
